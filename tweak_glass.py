...
for y in range(height - 1, -1, -1):
    for x in range(width):
        ...
        # Power of 24, but multiplier increased from 150.0 to 250.0 for even stronger edge refraction
        edge_dist_x = -math.pow(dx, 24) * dx * 250.0
        edge_dist_y = -math.pow(dy, 24) * dy * 250.0
...
# Increase CSS blur from 8px to 16px
html = html.replace('filter: blur(8px);', 'filter: blur(16px);')

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(html)

print("Blur and refraction increased.")
