...
for y in range(height - 1, -1, -1):
    for x in range(width):
        ...
        # Power of 24! Extremely sharp, creating a distinct "lining" at the edge.
        # Increased multiplier to 150 for stronger refraction at that lining.
        edge_dist_x = -math.pow(dx, 24) * dx * 150.0
        edge_dist_y = -math.pow(dy, 24) * dy * 150.0
...
# Reduce magnification from scale(3) to scale(1.8)
html = html.replace('transform: scale(3);', 'transform: scale(1.8);')

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(html)

print("Edge lining refraction increased, magnification reduced to 1.8x.")
