...
for y in range(height - 1, -1, -1):
    for x in range(width):
        ...
        # This makes the edges pull pixels from the INSIDE of the lens (convex)
        # instead of the OUTSIDE. This completely eliminates edge-clamping smears
        # because the displacement coordinates will never point outside the screen!
        edge_dist_x = -math.pow(dx, 4) * dx * 50.0
        edge_dist_y = -math.pow(dy, 4) * dy * 50.0
...
# Add a smooth map filter to eliminate 8-bit banding on the map itself
if 'result="smooth-map"' not in html:
    html = html.replace('result="map"/>', 'result="raw-map"/>\n    <feGaussianBlur in="raw-map" stdDeviation="2" result="smooth-map"/>')

# Change in2="map" to in2="smooth-map"
html = html.replace('in2="map"', 'in2="smooth-map"')

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(html)

print("Lens clamping fixed and map smoothed.")
